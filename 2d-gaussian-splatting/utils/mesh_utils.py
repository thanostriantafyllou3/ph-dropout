...
class GaussianExtractor(object):
    ...
    @torch.no_grad()
    def reconstruction(self, viewpoint_stack, render_only=False, dropout_ratio=0.0, num_trials=1):
        """
        ...
        for i, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="reconstruct radiance fields"):
            ...
            for trial in range(num_trials): # number of trials per image for a given dropout ratio
                render_pkg = self.render(viewpoint_cam, self.gaussians, render_only=render_only, dropout_ratio=dropout_ratio)
                rgb = render_pkg['render']
                alpha = render_pkg['rend_alpha']
                normal = torch.nn.functional.normalize(render_pkg['rend_normal'], dim=0)
                depth = render_pkg['surf_depth']
                depth_normal = render_pkg['surf_normal']
                rgb_trials.append(rgb.cpu())
                depth_trials.append(depth.cpu()) 
            self.rgbmaps.append(rgb_trials)
            self.depthmaps.append(depth_trials)
    ...
    def extract_mesh_bounded(self, voxel_size=0.004, sdf_trunc=0.02, depth_trunc=3, mask_backgrond=True):
        ...
        for i, cam_o3d in tqdm(enumerate(to_cam_open3d(self.viewpoint_stack)), desc="TSDF integration progress"):
            rgb = torch.mean(torch.stack(self.rgbmaps[i]), dim=0) # mean over trials
            depth = torch.mean(torch.stack(self.depthmaps[i]), dim=0) # mean over trials
        ...
        def compute_unbounded_tsdf(samples, inv_contraction, voxel_size, return_rgb=False):
            ...
            for i, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="TSDF integration progress"):
                sdf, rgb, mask_proj = compute_sdf_perframe(i, samples,
                    depthmap = torch.mean(self.depthmaps[i], dim=0),
                    rgbmap = torch.mean(self.rgbmaps[i], dim=0),
    ...
    def export_image(self, path):
        ...
        for idx, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="export images"):
            ...
            for trial_i, rgb in enumerate(self.rgbmaps[idx]):
                save_img_u8(rgb.permute(1,2,0).cpu().numpy(), os.path.join(image_dir, str(trial_i) + ".png"))
