...
def batchify(fn, chunk):
    ...
    if chunk is None:
        return fn
    def ret(inputs, dropout_ratio=0.0):
        return torch.cat([fn(inputs[i:i+chunk], dropout_ratio) for i in range(0, inputs.shape[0], chunk)], 0)
    return ret

def run_network(inputs, viewdirs, fn, embed_fn, embeddirs_fn, netchunk=1024*64,
                current_iter=None, reg_end_iter=None, dropout_ratio=0.0):
    ...
    outputs_flat = batchify(fn, netchunk)(embedded, dropout_ratio=dropout_ratio)
    outputs = torch.reshape(outputs_flat, list(inputs.shape[:-1]) + [outputs_flat.shape[-1]])
    return outputs, 1 - coord_freq_mask.mean().item()


def batchify_rays(rays_flat, chunk=1024*32, keep_keys=None, dropout_ratio=0.0, **kwargs):
    """Render rays in smaller minibatches to avoid OOM.
    """
    # ret_rgb_only = keep_keys and len(keep_keys) == 1 and keep_keys[0] == 'rgb_map'
    all_ret = {}
    for i in range(0, rays_flat.shape[0], chunk):
        ret = render_rays(rays_flat[i:i+chunk], dropout_ratio=dropout_ratio, **kwargs)
        for k in ret:
            if k == 'masking_ratio':
                continue
...
def render(H, W, focal, chunk=1024*32, rays=None, c2w=None, ndc=True,
    ...
    # Render and reshape
    all_ret = batchify_rays(rays, chunk, keep_keys=keep_keys, dropout_ratio=dropout_ratio, **kwargs)
    for k in all_ret:
        if k == 'masking_ratio':
            continue
...
def get_auto_dropout(render_poses, H, W, focal, chunk, ssim_decr_margin, gt_images=None, **kwargs):
    if gt_images is None:
        raise ValueError("gt_images must be provided when computing auto-dropout based on ssim_decr_margin.")
    if len(gt_images) != len(render_poses):
        raise ValueError("Number of ground truth images must match the number of render poses.")
    ...
    for i, c2w in enumerate(render_poses):
        rgb, disp, acc, _, _ = render(H, W, focal, chunk=chunk, c2w=c2w[:3,:4], dropout_ratio=0.0, **kwargs)
        rgb = rgb.detach().cpu().numpy()
        gt_image = gt_images[i]
        train_views_ssim_no_dropout.append(compare_ssim(rgb, gt_image, multichannel=True, data_range=1))
    ...
    while True:
        ...
        train_views_ssim = []
        for i, c2w in enumerate(render_poses):
            rgb, disp, acc, _, _ = render(H, W, focal, chunk=chunk, c2w=c2w[:3,:4], dropout_ratio=dropout_ratio, **kwargs)
            rgb = rgb.detach().cpu().numpy()
            gt_image = gt_images[i]
            train_views_ssim.append(compare_ssim(rgb, gt_image, multichannel=True, data_range=1))
        ...
        if avg_train_ssim <= (1 - ssim_decr_margin) * avg_train_ssim_no_dropout:
            ...
            return dropout_ratio, avg_train_ssim_no_dropout, avg_train_ssim


def render_path(render_poses, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0, gt_train_imgs=None, train_poses=None, ssim_decr_margin=-1.0, dropout_ratio=0.0, num_trials=1):
    H, W, focal = hwf
    ...
    if ssim_decr_margin >= 0:
        print(f'Train poses: {len(train_poses)}, Render poses: {len(render_poses)}')
        dropout_ratio, avg_train_ssim_no_dropout, avg_train_ssim = get_auto_dropout(train_poses, H, W, focal, chunk=chunk, ssim_decr_margin=ssim_decr_margin, gt_images=gt_train_imgs, **render_kwargs)
        # Log dropout ratio in a txt file
        with open(os.path.join(savedir, 'dropout_ratio.txt'), 'w') as f:
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        pose_rgbs = []
        pose_disps = []
        image_dir = os.path.join(savedir, '{0:05d}'.format(i))
        os.makedirs(image_dir, exist_ok=True)
        for trial in range(num_trials):
            rgb, disp, acc, _, _ = render(H, W, focal, chunk=chunk, c2w=c2w[:3,:4], dropout_ratio=dropout_ratio, **render_kwargs)
            rgb = rgb.detach().cpu().numpy()
            disp = disp.detach().cpu().numpy()
            pose_rgbs.append(rgb)
            pose_disps.append(disp)

            if savedir is not None:
                rgb8 = to8b(rgb)
                filename = os.path.join(image_dir, str(trial) + ".png")
                imageio.imwrite(filename, rgb8)
        
        rgbs.append(np.mean(pose_rgbs, axis=0))
        disps.append(np.mean(pose_disps, axis=0))
...
def create_nerf(args):
    ...
    network_query_fn_train = lambda inputs, viewdirs, network_fn, current_iter, reg_end_iter, dropout_ratio=0.0 : run_network(inputs, viewdirs, network_fn,
                                                                embed_fn=embed_fn,
...
def render_rays(ray_batch,
    ...
    raw, masking_ratio = network_query_fn(pts, viewdirs, network_fn, current_iter, reg_end_iter, dropout_ratio=dropout_ratio)
    ...
    if N_importance > 0:
        ...
        raw, _ = network_query_fn(pts, viewdirs, run_fn, current_iter, reg_end_iter, dropout_ratio=dropout_ratio)
...
def config_parser():
    ...
    parser.add_argument("--ssim_decr_margin", type=float, default=-1.0)
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            if args.render_test:
                ...
                test_images = None
            print(f'Train images shape: {train_images.shape}, Test images shape: {test_images.shape if test_images is not None else None}')
            
            testsavedir = os.path.join(basedir, expname, 'renderonly_{}_{:06d}'.format('test' if args.render_test else 'path', start))
            if args.ssim_decr_margin > 0.0:
                testsavedir += f'_ssim_decr_margin_{args.ssim_decr_margin}'
            else:
                testsavedir += '_dropoutratio_{:.2f}'.format(args.dropout_ratio)
            os.makedirs(testsavedir, exist_ok=True)
            print('test poses shape', render_poses.shape)

            rgbs, _ = render_path(render_poses, hwf, args.chunk, render_kwargs_test, gt_imgs=test_images, savedir=testsavedir, render_factor=args.render_factor, gt_train_imgs=train_images, train_poses=torch.Tensor(poses[i_train]).to(device), ssim_decr_margin=args.ssim_decr_margin, dropout_ratio=args.dropout_ratio, num_trials=args.num_trials)
            print('Done rendering', testsavedir)
            imageio.mimwrite(os.path.join(testsavedir, 'video.mp4'), to8b(rgbs), fps=30, quality=8)
            if args.render_test:
                # Compute metrics
                mse, psnr, ssim, lpips = get_perceptual_metrics(rgbs, test_images, device=device)
