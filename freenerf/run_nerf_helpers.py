...
class DenseLayer(nn.Linear):
    def __init__(self, in_dim: int, out_dim: int, activation: str = "relu", apply_dropout : bool = False, *args, **kwargs) -> None:
...
class NeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, skips=[4], use_viewdirs=False):
        ...
        self.pts_linears = nn.ModuleList(
            [DenseLayer(input_ch, W, activation="relu", apply_dropout=True)] + [DenseLayer(W, W, activation="relu") if i not in self.skips else DenseLayer(W + input_ch, W, activation="relu") for i in range(D-1)])
    ...
    def trunk_pts(self, input_pts, dropout_ratio):
        h = input_pts
        for i, layer in enumerate(self.pts_linears):
            h = layer(h)
            if layer.activation == "relu":
                h = F.relu(h)
            elif layer.activation == "sine":
                h = torch.sin(h)
            if layer.apply_dropout:
                h = self.apply_dropout(h, dropout_ratio)
            if i in self.skips:
    ...
    def apply_dropout(self, x, dropout_ratio):
        assert 0 <= dropout_ratio < 1, "dropout_ratio must be in the range [0, 1)"
        if dropout_ratio == 0:
            return x
        dropout_mask = torch.rand_like(x) > dropout_ratio
        x = x * dropout_mask / (1.0 - dropout_ratio)
        return x

    def forward(self, x, dropout_ratio=0.0):
        input_pts, input_views = torch.split(x, [self.input_ch, self.input_ch_views], dim=-1)
        
        h = self.trunk_pts(input_pts, dropout_ratio)
