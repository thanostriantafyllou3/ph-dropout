# GSDropout
Predictive Uncertainty Quantification of 3D Gaussian Splatting using Inference-only Dropout.

Experiments are performed on [NeRF synthetic data](https://drive.google.com/file/d/18JxhpWD-4ZmuFKLzKlAw-w5PpzZxXOcG/view?usp=share_link) with different number of training views (8, 16, 100). [Click here](https://uoe-my.sharepoint.com/:f:/g/personal/s2261330_ed_ac_uk/EogIJ-KeHfdKjKFM4KdVNd4BG_Pmw1Oa7STAEF8jkiuhmQ?email=chuanhao.sun%40ed.ac.uk&e=jAX0oV) to see our current experimental results.

## Overview
GSDropout extends the original Gaussian Renderer (`guassian_renderer/__init__.py/render`) by creating a "dropout" mask of randomly assigned 0s and 1s such that `dropout_ratio` Gaussians are concealed during rendering. 

To accomplish that we apply the aforementioned mask on the opacity tensor (`pc.get_opacity`) which stores the opacities for each of the gaussian kernels. Notably, the "dropout" mask is applied when rendering during inference ONLY.

The predictive uncertainty for a given viewpoint is quantified according to the "standard deviation" image (std image); the standard deviation at each pixel of the std image is calculated using \[`num_trials`\] samples rendered using GSDropout for that given viewpoint. 

Then, the predictive uncertainty mask is the std image filtered above the following threshold; `std_mask_ratio * avg_max_stds_train` where `std_mask_ratio` is a hyperparameter (in range \[0,1\]) and `avg_max_stds_train` the average of the maximum standard deviations for each image in the train set.

The following is a visual example of GSDropout (`dropout_ratio=0.1`, `std_mask_ratio=0.5`) applied to the _chair_ NeRF synthetic dataset given 8 training views `[2, 16, 26, 55, 73, 75, 86, 93]` and 25 test views uniformly obtained from the chair's complete dataset:
...
```
# HTTPS
git clone https://github.com/thanostriantafyllou3/GSDropout.git
```
or
```
git clone git@github.com:thanostriantafyllou3/GSDropout.git
```

## Conda Environment Setup
```
conda env create -f environment.yml
conda activate GSDropout
```
...
1. On `train_test_split.py`:
   * Edit `train_imgs`, `test_imgs`, to specify which train and test images, respectively, of a given dataset (`src_dir`) to be saved on a new directory (`dst_dir`). This new directory will be used for the current experiment.

2. Train the 3D Gaussian Splatting as you would train [gaussian-splatting](https://github.com/graphdeco-inria/gaussian-splatting):
```
python train.py -s <path_to_dataset_supdir/dataset_dir_name> --eval -m output/<dataset_dir_name>
```

3. Render `num_trials` trials for each train/test image, by a applying a specified `dropout_ratio`,  using the trained model:
```
python render.py -m output/<dataset_dir_name> --dropout_ratio <dropout_ratio> --num_trials <num_trials>
```
_Alternatively_, given the average SSIM of the rendered train views without dropout (`avg_train_ssim_no_dropout`), specify (`ssim_decr_margin`) that achieves a `dropout_ratio` (2 d.p.) such that the average SSIM of the rendered train views with that dropout ratio (`avg_train_ssim`) satisfies the constraint; `avg_train_ssim <= (1 - ssim_decr_margin) * avg_train_ssim_no_dropout` (`ssim_decr_margin` in range (0,1)):
```
python render.py -m output/<dataset_dir_name> --ssim_decr_margin <ssim_decr_margin> --num_trials <num_trials>
```
_Note_ that only one of the `dropout_ratio` or `ssim_decr_margin` arguments must be specified on an experiment. Otherwise, an error will be raised.

4. Visualise results using `results.ipynb` template by executing: (_the resulted notebook is saved within_ the `output/<dataset_dir_name>` directory.)
```
# TODO: Set the variables
dataset="<dataset_dir_name>"
num_iterations="<num_of_training_iterations>"
dropout_ratio="<dropout_ratio>"
std_mask_ratio="<specify_a_std_mask_ratio>"

# Constructs the output notebook path
output_notebook="output/${dataset}/results_iter${num_iterations}_dropout${dropout_ratio}_stdmaskratio${std_mask_ratio}.ipynb"

# Prints output notebook path on terminal
echo $output_notebook

# Runs the notebook given the specified variables and saves it in $output_notebook
papermill results.ipynb "${output_notebook}" -p dataset "${dataset}" -p dropout_ratio "${dropout_ratio}" -p num_iterations "${num_iterations}" -p std_mask_ratio "${std_mask_ratio}"
```

**Running multiple experiments at once**: To run multiple experiments (on different train/test datasets) bash scripts have been created based on the Steps 2-4, above. These are stored in `scripts/`. After adjusting the variables within each bash script according to your needs you can run them in the following way; `train.sh`, `render.sh`, `results.sh`. Optionally, you can convert the notebooks to html which are stored separately based on `<dataset_dir_name>` in `results_html/`.


GSDropout is implemented on top of [gaussian-splatting](https://github.com/graphdeco-inria/gaussian-splatting):
